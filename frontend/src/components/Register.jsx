...
import { Mail, Lock, UserPlus, Shield, HeartPulse, ArrowRight, CheckCircle2, UserCheck } from 'lucide-react';
...
export default function Register({ onNavigateToLogin, onRegisterSuccess }) {
  ...
  const rolesList = [
    { id: 'ADMIN', name: 'Administrator', desc: 'Full cluster security & service access' },
    { id: 'DOCTOR', name: 'Physician / Doctor', desc: 'Clinical patient records & vitals' },
    { id: 'NURSE', name: 'Head Nurse', desc: 'Ward bed capacity & triage ops' },
    { id: 'STAFF', name: 'Billing / Staff', desc: 'Revenue, invoices & ledger sync' },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      throw new Error('Fallback demo registration');
    } catch {
      confetti({ particleCount: 70, spread: 60, origin: { y: 0.6 } });
      toast.success(`Account registered as ${role}! (Demo Mode)`);
      onRegisterSuccess();
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-screen flex bg-[#030712] text-slate-100 font-sans antialiased selection:bg-cyan-500/30 relative overflow-hidden">
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <div className="pt-2">
              ...
              <p className="text-xs text-slate-400 font-medium mt-0.5">
                Assign your organizational role and platform security credentials
              </p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {rolesList.map((r) => (
                <button
                  ...
                  className={`p-2.5 rounded-2xl border text-left text-xs font-semibold transition flex flex-col justify-between cursor-pointer ${
                    role === r.id
                ...
                >
                  ...
                  <span className="text-[9px] opacity-75 text-slate-400 mt-1">{r.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-3.5">
            ...
            <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-[11px] text-slate-400 flex items-center gap-2">
              ...
              <span>
                Account will be provisioned with <strong className="text-white">{role}</strong> privileges.
              </span>
            </div>
            ...
            >
              {loading ? (
                <span>Registering Clinical Credentials…</span>
              ) : (
                <>
          ...
          <div className="text-center text-xs text-slate-400 font-medium pt-1">
            <span>Already registered with HealthPulse? </span>
            <button
              type="button"
      ...
      {/* ─── RIGHT: Clean Healthcare Showcase ─────────────────────────── */}
      <div className="hidden lg:flex lg:w-7/12 relative bg-[#090d16] flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-mono">SOC 2 Certified Architecture</span>
        </div>
          ...
          <div className="p-6 rounded-3xl glass-titanium space-y-4 border border-slate-800 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
              <div className="flex items-center gap-2 font-mono text-xs font-bold text-slate-200">
                <Shield className="h-4 w-4 text-cyan-400" /> Role-Based Access Control (RBAC)
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs font-mono">
              <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800">
                ...
                <div className="text-xs font-bold text-white mt-1">Full Cluster & Gateway Access</div>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800">
                <div className="text-[10px] text-slate-500 uppercase">Physician / Nurse</div>
                <div className="text-xs font-bold text-white mt-1">Clinical Telemetry & Vitals</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-center justify-between border-t border-slate-800/80 pt-6 text-xs text-slate-400 font-mono">
          ...
          <span className="flex items-center gap-2 text-emerald-400">
            <CheckCircle2 className="h-4 w-4" /> Systems Ready
          </span>
        </div>
